...
- `load_specs()` in `register.py` lazily registers the shipped specs from
  `apps_v2.blocks.tables`.
...
### 6.3 Demo Views
- `GET /blocks/v2/table/layouts` and `/blocks/v2/table/items` render the shipped
  table specs through `BlockController` for manual testing.
