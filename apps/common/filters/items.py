...
def format_item_choices(qs, limit: int = 20) -> List[Tuple[str, str]]:
...
def make_item_choices_for_queryset(qs_provider: Callable[[object], "models.QuerySet[Item]"]):
    ...
    def _choices(user, query: str = "") -> List[Tuple[str, str]]:
        qs = _apply_item_search(qs_provider(user), query)
...
def item_choices(user, query: str = "") -> List[Tuple[str, str]]:
    ...
    qs = _apply_item_search(Item.objects.all(), query)
    return format_item_choices(qs)

# START ADDING FROM HERE
def item_choices_for_open_po(user, query: str = "") -> List[Tuple[str, str]]:
