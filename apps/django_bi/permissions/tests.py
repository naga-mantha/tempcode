...
from apps.django_bi.permissions.signals.generate_field_permissions import (
    generate_field_permissions,
)
from apps.django_bi.permissions.utils import generate_field_permissions_for_model
...
class RebuildFieldPermissionsCommandTests(TestCase):
    ...
    @patch(
        "apps.django_bi.permissions.management.commands.rebuild_field_permissions.generate_field_permissions_for_model",
        return_value=(0, 0),
    )
...
class ProxyAndAbstractModelTests(TestCase):
    def test_returns_zero_for_proxy_model(self):
        ...
        with patch(
            "apps.django_bi.permissions.utils.ContentType.objects.get_for_model"
